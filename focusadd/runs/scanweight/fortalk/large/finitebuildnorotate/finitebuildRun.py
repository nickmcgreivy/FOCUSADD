...
def setArgs():
	...
	parser.add_argument("-nz","--numZeta", help="Number of gridpoints in zeta (toroidal angle) on the magnetic surface", default=128)
	...
	parser.add_argument("-ln","--lengthNormal", help="Length between each coil in the (rotated) normal direction", default=0.03)
	parser.add_argument("-lb","--lengthBinormal", help="Length between each coil in the (rotated) binormal direction", default=0.03)
	...
	parser.add_argument("-o" ,"--outputFile", help="Name of output file for coils", default="finitebuildData")
	parser.add_argument("-i" ,"--inputFile", help="Name of input file for coils", default=None)
	parser.add_argument("-w" ,"--weightLength", help="Length of weight paid to coils", default=0.001)
...
def endRun(output_file,loss_vals,loss_vals_filament,coilSet,params):
...
def main():
	...
	zeroRotateParam = np.zeros(param1_filament.shape)
	...
	try:
		...
		for i in range(int(args.numIter)):
			...
			param0, _ = params
			loss_vals_filament.append(l_filament.loss((param0,zeroRotateParam)))
		...
		endRun(output_file, loss_vals, loss_vals_filament,coilSet,params)
		raise e	
	end = time.time()
	print(end - start)
