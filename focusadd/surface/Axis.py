...
class Axis:
	...
	def compute_x3y3z3(self):
		x3 = np.zeros(self.N_zeta)
		y3 = np.zeros(self.N_zeta)
		z3 = np.zeros(self.N_zeta)
		for m in range(len(self.xc)):
			arg = m * self.zeta
	...
	def compute_torsion(self):
		...
		top = cross12[:,1] * r3[:,1] + cross12[:,2] * r3[:,2] + cross12[:,3] * r3[:,3]
		bottom = np.linalg.norm(cross12,axis=1)
		self.torsion = top / bottom[:,np.newaxis]
